...
The `ml-framework-snapshots` tool verifies that `zero-orbax` accurately replicates the public API signatures, parameters, and defaults of the real `orbax` framework.

**To run the checker against the current codebase:**

```bash
ml_framework_snapshots check orbax ~/repos/zero-orbax/src \
    --reference-prefix orbax \
    --target-prefix zero_orbax
```

**What it does:**
- It statically analyzes all Python modules in `zero-orbax/src`.
- It maps the target namespace (`zero_orbax.*`) back to the reference framework (`orbax.*`).
...
```bash
ml-switcheroo-ir compliance ~/repos/zero-orbax/src
```
